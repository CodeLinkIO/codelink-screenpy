...
class MetadataEditPage(BasePage):
    ...
    # VALIDATE_BUTTON = Target.the('Validate Button').located_by("//div[contains(@class, 'MetadataTable__TableContent')]/div[1]//button[contains(@class, 'ValidationButton')]")
    ...
    UPLOAD_THUMBNAIL_INPUT = Target.the('Upload Thumbnail Input').located_by("[class^=Modal__Overlay] input[type=file]']")
    THUMBNAIL_IMAGE_PREVIEW = Target.the('Thumbnail Image Preview').located_by("[class^=SelectableImage__PreviewModal]")
